...
const PAPERS = [
  ...
  },
  {
    ...
    status: { emoji: '🔄', label: 'Under Review', className: 'bg-amber-50 text-amber-700 border border-amber-200' },
  },
  {
    id: 3,
    tag: 'VR / HCI',
  ...
  },
  {
    ...
    status: { emoji: '🔭', label: 'Ongoing', className: 'bg-stone-100 text-stone-600 border border-stone-300' },
  },
]
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 mb-4">
        <span className="px-3 py-1 bg-accent-soft border border-accent/15 text-accent text-xs font-bold tracking-wide rounded-full">
          {paper.tag}
        </span>
      ...
      {/* Status badge */}
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full w-fit ${paper.status.className}`}>
      ...
      <button
        ...
        className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-accent hover:text-accent/75 transition-colors"
    ...
    <section id="research" className="py-28 px-6 bg-warm-bg">
      <div className="max-w-5xl mx-auto">
        <motion.div
          ...
          className="mb-16"
        >
          <p className="text-xs font-bold tracking-[0.3em] text-accent uppercase mb-3">Research</p>
